...
def main() -> None:
    ...
    # Check before connecting because SQLite can create an empty file.
    if not DB_PATH.exists():
        fail(["roadmap.db does not exist"])

    evaluation_files = sorted(
        EVALUATIONS_DIR.glob(EVALUATION_PATTERN)
    )
    ...
    try:
        ...
        if current_count != 1:
            errors.append(
                "current_context should have exactly 1 row, "
        ...
        if active_skills != profile_rows:
            errors.append(
                "profile_skill_state should contain one row "
                "per active skill: "
                f"{profile_rows} profile rows for "
                f"{active_skills} active skills"
            )

        if "next_diagnostic_id" in column_names(
            conn,
            "profile_skill_state",
        ):
            errors.append(
                "profile_skill_state still contains obsolete "
                "next_diagnostic_id"
            )

        attempt_table = fetch_all(
            conn,
            """
            SELECT name
            FROM sqlite_master
        ...
        if attempt_table:
            errors.append(
                "obsolete diagnostic_attempts table still exists"
            )

        planned_identity_table = fetch_all(
            conn,
            """
            SELECT name
            FROM sqlite_master
        ...
        if planned_identity_table:
            errors.append(
                "obsolete diagnostic_skill_clusters table still exists"
            )

        orphaned_state_events = fetch_all(
            conn,
            """
            SELECT
                ps.skill_cluster_id,
        ...
        if orphaned_state_events:
            errors.append(
                "Profile rows have missing or mismatched "
                f"last events: {orphaned_state_events}"
            )

        diagnostics_without_results = fetch_all(
            conn,
            """
            SELECT d.id
            FROM diagnostics d
        ...
        if bad_ratings:
            errors.append(
                f"Invalid profile ratings: {bad_ratings}"
            )

        duplicate_thresholds = fetch_all(
            conn,
            """
            SELECT
                target_id,
        ...
        if duplicate_thresholds:
            errors.append(
                f"Duplicate target thresholds: {duplicate_thresholds}"
            )
    ...
    print(
        ...
        f"{len(evaluation_files)} evaluation files and "
        f"{diagnostic_count} diagnostics found."
    )
